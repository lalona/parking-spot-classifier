...
def main():
		...
		dataset_directory = ntpath.basename(dataset_filename).split('.')[0] + "_dim-{}".format(image_dim)
		...
		for image_info in tqdm(images_info_reduced):
				...
				image = preprocessImage(os.path.join(path_patches, image_info['filePath']), image_dim)
				image = np.array(image, dtype="float") / 255
				filename = ntpath.basename(image_info['filePath'])
